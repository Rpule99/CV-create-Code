...
window.addEventListener('load', (ev) => {
    //Check number of CV's in the mix and make the cvload wait till we done
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").set({
        AwaitLoad: false,
        SecondViewing: false
    });
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").get().then(function (snapshot) {
        var numCVs = snapshot.size;
        if (numCVs != 0) {
            AddBasicEducation(numCVs);
            AddHigerEducation(numCVs);
            AddDriving(numCVs);
            AddFirstAid(numCVs);
            AddTravel(numCVs);
            AddJobs(numCVs);
            AddComServ(numCVs);
            displayInfodata(numCVs);
            Screenshots(numCVs)
        } else {
            AddBasicEducation(1);
            AddHigerEducation(1);
...
function Screenshots(n) {
    CvId = "CV" + n;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("Export").doc("status").set({
        Exported: false,
        rendered: false
    });
...
//function to show add higher education Modal
function QualHeModal() {
    $('#HigherEdModal').modal('show');
}
//function to add qual to wizard
function addBasicEd(a) {
    var CVID = "CV" + a;
    ...
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CVID).collection("DocumentED").doc("BE3").set({
...
//function to add HigherEducation to wizard
function addHigherEd(a) {
    var CVID = "CV" + a;
    var container = document.getElementById("TableBodyHigh");
    //add details
    //first row <tr>
    var FirstRow = document.createElement("tr");
    FirstRow.setAttribute("id", "FirstRow");
    ...
    //add education
    //first store values
    var SchoolName = document.getElementById("InstitutionNameMod").value;
    var HGP = document.getElementById("HEPnameMod").value;
    var yearStarted = document.getElementById("EnrolYearMod").value;
    var yearFinished = document.getElementById("LeavingYearMod").value;
    //minipulate values and them to table
    var yearstartStore = yearStarted.toString();
    var yearFinishedStore = yearFinished.toString();
    var yearstartarray = yearstartStore.split("-");
...
function ChangeAddtoCV(n, y) {
    var CvId = "CV" + y;
    var TheDoc = "BE3"
    var checkBox = getValue("DynamicCheckBox");
    if (checkBox.checked == true) {
        db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("DocumentED").doc(TheDoc).update({
            addToCV: true
        });
    } else {
        db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("DocumentED").doc(TheDoc).update({
...
function AddHigerEducation(a) {
    db.collection("Jobseeker").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("HigherEducation").get().then(function (snapshot) {
        ...
        if (snapshot.empty == false) {
            ...
            //for loop to add the elements
            for (let index = 1; index <= numDoc; index++) {
                createExistingHE(index, a);
            }
        }
        // add button
        var addbutton = document.createElement("button");
        addbutton.setAttribute("id", "AddHEButton");
        addbutton.setAttribute("class", "btn btn-primary");
        addbutton.setAttribute("onclick", "QualHeModal()");
        var ButtonText = document.createTextNode("Add");
        addbutton.appendChild(ButtonText);
        //addbutton.setAttribute("onclick","addBE()");
        container.appendChild(addbutton);
...
function createDriving(n, x) {
    ...
    var Checkbox = document.createElement("input");
    Checkbox.setAttribute("type", "checkbox");
    Checkbox.setAttribute("onclick", "DrivingAddtoCV(" + n + ", " + x + ")");
    Checkbox.setAttribute("class", "form-check-input");
    ...
    db.collection("Jobseeker").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("Driving").doc(Doc).get().then(function (doc) {
        ...
        Driving2CV(type, Code, expiration, Doc, x);
    });
    //add the info to column

}
//add license to CV
function Driving2CV(name, code, years, numberOf, CVNum) {
    var CvId = "CV" + CVNum;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("Documentation").doc(numberOf).set({
        Name: name,
        No: code,
        Yearz: years,
        addToCV: false
...
//check license to CV
function DrivingAddtoCV(n, y) {
    var CvId = "CV" + y;
    var TheDoc = "D" + n;
    var checkboxID = "CheckBoxD" + n;
    var checkbox = document.getElementById(checkboxID);
    if (checkbox.checked == true) {
        db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("Documentation").doc(TheDoc).update({
            addToCV: true
        });
    } else {
        db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("Documentation").doc(TheDoc).update({
...
function createTravel(n, x) {
    ...
    var Checkbox = document.createElement("input");
    Checkbox.setAttribute("type", "checkbox");
    Checkbox.setAttribute("onclick", "TravelAddtoCV(" + n + ", " + x + ")");
    Checkbox.setAttribute("class", "form-check-input");
    ...
    db.collection("Jobseeker").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("Travel").doc(Doc).get().then(function (doc) {
        ...
        var expiration = yearStarted + "-" + yearFinished;
        Travel2CV(passportType, passportnum, expiration, Doc, x);
    });
    //add the info to column

}
//add travel(passport) to CV
function Travel2CV(passName, passNo, Years, numberOf, CVnum) {
    var CvId = "CV" + CVnum;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("Documentation").doc(numberOf).set({
        Name: passName,
        No: passNo,
...
function createJob(n, x) {
    ...
    Checkbox.setAttribute("onclick", "JobAddtoCV(" + n + ", " + x + ")");
    col2.appendChild(Checkbox);
    FirstRow.appendChild(col2);
    //now to get data from databse and add
    //add education
    ...
    db.collection("Jobseeker").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("Jobs").doc(Doc).get().then(function (doc) {
        ...
        Jobs2CV(SchoolName, HGP, completeYears, Doc, x);
    });
    //add the info to column
}
//Add jobs to the CV
function Jobs2CV(School, Grade, Years, numberOF, CVnumber) {
    var CvId = "CV" + CVnumber;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("DocumentEXP").doc(numberOF).set({
        JobTitle: School,
        Organisation: Grade,
        years: Years,
        addToCV: false
...
function JobAddtoCV(n, x) {
    var CvId = "CV" + x;
    var TheDoc = "J" + n;
    var checkboxID = "CheckBoxjob" + n;
    var checkbox = document.getElementById(checkboxID);
    if (checkbox.checked == true) {
        db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("DocumentEXP").doc(TheDoc).update({
            addToCV: true
        });
    } else {
        db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("DocumentEXP").doc(TheDoc).update({
...
function createExistingBE(n, x) {
    ...
    Checkbox.setAttribute("class", "form-check-input");
    Checkbox.setAttribute("onclick", "BEAddtoCV(" + n + ", " + x + ")");
    col2.appendChild(Checkbox);
    FirstRow.appendChild(col2);
    //now to get data from databse and add
    //add education
    ...
    db.collection("Jobseeker").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("BasicEducation").doc(Doc).get().then(function (doc) {
        ...
        document.getElementById(YearsNest).innerHTML = yearStarted + "-" + yearFinished;
        var completeYears = yearStarted + "-" + yearFinished;
        BasicEd2CV(SchoolName, HGP, completeYears, Doc, x);
    });
    //add the info to column
...
function BasicEd2CV(School, Grade, Years, numberOF, CVnum) {
    var CvId = "CV" + CVnum;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("DocumentED").doc(numberOF).set({
        schholName: School,
        grade: Grade,
        years: Years,
        addToCV: false
...
function createExistingHE(n, x) {
    ...
    Checkbox.setAttribute("class", "form-check-input");
    Checkbox.setAttribute("onclick", "HEAddtoCV(" + n + ", " + x + ")");
    col2.appendChild(Checkbox);
    FirstRow.appendChild(col2);
    //now to get data from databse and add
    //add education
    ...
    db.collection("Jobseeker").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("HigherEducation").doc(Doc).get().then(function (doc) {
        ...
        document.getElementById(YearsNest).innerHTML = yearStarted + "-" + yearFinished;
        var CompleteYears = yearStarted + "-" + yearFinished;
        HigherEd2CV(SchoolName, HGP, CompleteYears, Doc, x);
    });
    //add the info to column

}
//add higher education to CV
function HigherEd2CV(School, Grade, Years, numberOF, CVnum) {
    var CvId = "CV" + CVnum;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).collection("DocumentED").doc(numberOF).set({
        schholName: School,
        grade: Grade,
...
async function displayInfodata(a) {
    ...
    db.collection("Jobseeker").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").get().then(function (doc) {
        ...
        AddContactInfo(name.value, surname.value, email.value, phonenum.value, province.value, area.value, a);

    });
}
//add contact information to cv data
async function AddContactInfo(name, surname, email, phone, dob, area, CVnumber) {
    ...
    var aboutme = ' ';
    var Jobrole = 'Domestic Worker';
    var CvId = "CV" + CVnumber;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(CvId).set({
        Name: ContactName,
...
function AddAboutDescription() {
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").get().then(function (snapshot) {
        var CVnumberz = snapshot.size;
        var CvId = "CV" + CVnumberz;
        var messageBox = document.getElementById("writeBox").value;
        Addaboutmetocv(CvId);
        function Addaboutmetocv(x) {
            db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(x).update({
                AboutMe: messageBox
            });
        }
    });
...
//change surname
function ChangeSurnameCV() {
    var feild = getValue("Surname").value;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc("CV").update({
        Surname: feild
    });
}
...
function ChangeEmail() {
    var feild = getValue("Email").value;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc("CV").update({
        Email: feild
    });
}
...
function ChangeDOB() {
    var feild = getValue("Province").value;
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc("CV").update({
        DOB: feild
    });
}
...
function ExportCV() {
    db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").get().then(function (snapshot) {
        ...
        function changeExport(n) {
            db.collection("test").doc("sDHiAGDFbrVjC6wO2RhwxD0HGEC2").collection("testCV").doc(n).collection("Export").doc("status").update({
                Exported: true
            });
            setTimeout(function () { window.location.href = 'https://vitawizmycv.web.app/'; }, 50000);
        }
